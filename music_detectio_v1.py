...
class UltraLowLatencyAudioMonitor:
    ...
    """ menjalankan program lua run_setup.lua"""
    def run_lua_script(self):
        lua_script = "run_setup.lua"
        try:
            subprocess.run(["lua", "run_setup.lua"], check=True)
            print("✅ Lua script executed successfully.")
        except subprocess.CalledProcessError as e:
            print(f"❌ Lua script execution failed: {str(e)}")
        ...
        try:
            with self.loopback.recorder(
                ...
                while self.running:
                    ...
                    # 2. Konversi ke mono secara ultra-cepat dengan one-liner
                    ...
                    # Deteksi musik
                    if vol_db > -30 and not self.music_detected:
                        self.music_detected = True
                        self.run_lua_script()

                    #reset status jika volume turun
                    if vol_db <= -30:
